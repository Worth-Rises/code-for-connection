...
export default function App() {
    const { user } = useAuth();
    return (<div className="tablet-safe-area bg-gray-100">
      <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-blue-900">
            <LoadingSpinner size="lg"/>
          </div>}>
        <Routes>
          <Route path="/login" element={user ? <Navigate to="/incarcerated" replace/> : <PinLoginPage />}/>
          <Route path="/incarcerated" element={<RequireAuth>
                <IncarceratedLayout />
              </RequireAuth>}>
