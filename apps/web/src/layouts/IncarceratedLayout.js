    ...
    return (<div className="min-h-screen bg-gray-100 flex flex-col">
      <header className="bg-blue-900 text-white px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <button onClick={handleLogout} className="px-4 py-2 bg-blue-800 hover:bg-blue-700 rounded-lg text-sm">
            Logout
          </button>
        </div>
      </header>
      ...
      <nav className="bg-white border-b border-gray-200">
        <div className="flex">
          {navItems.map((item) => (<NavLink key={item.path} to={item.path} className={({ isActive }) => `flex-1 px-4 py-4 text-center border-b-2 transition-colors ${isActive
