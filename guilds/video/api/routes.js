...
exports.videoRouter.get('/check-limit/:incarceratedPersonId', shared_1.requireAuth, async (req, res) => {
    try {
        ...
        const weeklyRequests = await shared_1.prisma.videoCall.count({
            where: {
...
exports.videoRouter.post('/record-usage/:incarceratedPersonId', shared_1.requireAuth, async (req, res) => {
    try {
        const { incarceratedPersonId } = req.params;
        ...
        res.json((0, shared_1.createSuccessResponse)({ recorded: true, usage }));
    }
    catch (error) {
